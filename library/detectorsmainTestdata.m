function [output] = detectorsmainTestdata(Data, detectorName)
...
for l = 1:numel(params.combo) 
    ...
    % for k = 1:size(Data.(fieldList{1}).finalEMGdata,2)
                    ...
                    % end  
            %% Call the detector
            Dataparams.dur = length(Data  )/Dataparams.fs;
            detectorfunc = str2func(strcat('faster',detectorName));
            output{1,l} = detectorfunc(Data  ,params.combo{l}, params, Dataparams);
