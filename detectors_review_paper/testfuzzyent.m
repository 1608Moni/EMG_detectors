...
SNR        = -3;
type       = {'gaussian'};
field      = strcat(detector(1),'trail',num2str(trail),...
...
ylim([-0.1 1.1])
xlim([0, 13]);
% l= legend([lh,p1,p2,p3,p6],'$EMG_{rectified}$   ($|x[n]|$)','$EMG_{LPF}$   ($g[n]$)','$Mean_{baseline}$   ($\mu_{g}$)','Threshold   (h)','Output   (y[n])')
% set(legend,'Orientation','vertical',...
%     'Interpreter','latex','FontSize',10,'EdgeColor',[1 1 1], 'Box', 'off');
xlabel('Time (s)');
ylabel('Amplitude')
set(gca,"FontSize",15)
...
text(8.1,2.4,strcat('$f_{{\nabla}t} = $',num2str(round(f_delT,3))),'Interpreter','Latex','FontSize',15)
%%
